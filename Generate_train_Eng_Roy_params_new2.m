function [train isok sizes]=Generate_train_Eng_Roy_params_new2(params,N,m)
lambdafunc=@(x) -m.*x+1.0581
...
%            MS_t=[ MS_t iter]; %store the "time"
            ...
            for w=1:L
                for ww=1:L
                    potmap(w,ww)=lambdafunc(sizes(MS_t_counter))*L*(((w-i0)/i0)^2+((ww-j0)/j0)^2);
                end
            end
