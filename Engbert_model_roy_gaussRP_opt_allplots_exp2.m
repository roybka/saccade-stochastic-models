...
s_params{2}=[7.8,20,200]; % [7.83,51,190]
s_params{7}=[7.75,12,170]; % [7.83,51,190]
s_params{8}=[7.83,24,170];
hc=s_params{snum}(1);%7.83;
s_params{snum}
mu_sigma(1,:) = [51,190];%[35 107] %          s8 - [24,170]
...
% 
% subplot(3,1,3)
%load 'C:\Users\Owner\Google Drive\Roy- experiments\checkerMicSac\dark_ms_v3\GA10'
%ex_gauss_params=[mean(mus1(snum,:)),mean(sigs(snum,:)),mean(lambdas(snum,:))]



% figure
% plot(1:3000,smooth_trace(1,:)./sum(smooth_trace(1,:)),'b')
% p = exgausspdf(ex_gauss_params(1), ex_gauss_params(2), ex_gauss_params(3), 0:3000);
% hold on;plot(p,'r--')
% legend('model','theory')
cutoff=3000;
d2=d(d<cutoff);
...
smooth_trace=smoothy(trace,50);
% figure;
% plot(1:cutoff,smooth_trace./sum(smooth_trace))
% p = exgausspdf(ex_gauss_params(1), ex_gauss_params(2), ex_gauss_params(3), 0:cutoff);
% hold on;plot(p,'r')
% legend('model','theory')
% figure
% hist(d2,50)
...
pd=smooth_trace./sum(smooth_trace);
cd=cumsum(pd);
su=1-cd;
haz=pd./su;
...
%load 'C:\Users\Owner\Google Drive\Roy- experiments\checkerMicSac\dark_ms_v3\GA10';
%pars=[mean(mus1(snum,:)),mean(sigs(snum,:)),mean(lambdas(snum,:))];
% p = exgausspdf(ex_gauss_params(1), ex_gauss_params(2), ex_gauss_params(3), 0:cutoff);
% 
%     plot(p);
%     hold off
    ...
    figure;plot(smoothy(ACG./sum(ACG),50),'r')
    xlim([0 1500])
    
    a=mean([norm_sacarlow(snum,:);norm_sacarhigh(snum,:)]);
    a=norm_sacarhigh(snum,:);
hold on;plot(smoothy(a,50),'b')
legend('model','data')
xlim([0 1500])
title(['s' num2str(snum) 'mu=' num2str(mu1) 'sigma=' num2str(sigma1) 'nsacs per run=' num2str(n_sacs/runs) 'HC=' num2str(hc)]);
    dat=1
    if dat
        
        k=snum
        figure
        realdat=([normalizedsubs{k}(1,1:3000)]);%;normalizedsubs{k}(2,1:1500)])
        
        plot(smoothy(realdat,100))
        hold on
        plot(1:cutoff,smooth_trace./sum(smooth_trace),'r')
        legend('data','model')
        title(['s' num2str(snum) 'mu=' num2str(mu1) 'sigma=' num2str(sigma1) 'nsacs per run=' num2str(n_sacs/runs) 'HC=' num2str(hc)]);
