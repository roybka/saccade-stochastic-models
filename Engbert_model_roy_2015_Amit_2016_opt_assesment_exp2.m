...
for lambda1=1.48*2;%1.48; %after micro-saccade
lambda2=9.07*0.06; %9.07%after micro-saccade
rho1=0.0052*0.5*0.5*.8;%0.0052; %after micro-saccade
...
%ex_gauss_params=[141.9822, 44.3091, 384.5009]
p = exgausspdf(ex_gauss_params(1),ex_gauss_params(2),ex_gauss_params(3), 1:3000);

rate=length(d)/(180*runs);
figure
plot(p,'r');
hold on
plot(smooth_trace./sum(smooth_trace),'b')
title(['rate ' num2str(rate) 'rho1=' num2str(rho1) 'rho2=' num2str(rho2) 'lambda1=' num2str(lambda1) 'lambda2=' num2str(lambda2) 'hc=' num2str(hc)])
legend('theorhetical','model')
xlim([0 1500])

likely=eglike(ex_gauss_params,d2)/length(d2);



sols(gcnt,:)=[hc rho1 rho2 lambda1 lambda2 rate likely trace];
solspops{gcnt}= d2;
end
end

%hc_t =  (1/(1 + lambda2*exp(rho2*[1:1000].^2)));
%% autocorrelation
ACG=zeros(1,2000);
...
for i=1:length(MSTIM{k})-7
 ...
 k=2
figure;plot(smoothy(ACG./sum(ACG),100),'r')

    

a=mean([norm_sacarlow(k,:);norm_sacarhigh(k,:)]);
hold on;plot(smoothy(a,100),'b')
xlim([0 1500])
        legend('model','data')
        ...
        figure
        realdat=([normalizedsubs{k}(1,1:1500)]);%normalizedsubs{k}(2,1:1500)])
